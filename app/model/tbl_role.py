...
class TblRole(Base):
    ...
    is_admin = Column(SmallInteger, name='F_is_admin', comment='是否是管理员 enum:0,normal_user,普通用户#1,admin,管理员用户', nullable=False, default=0)
    status = Column(SmallInteger, name='F_status', comment='角色状态 enum:0,normal,正常#1,ensure,待审核#2,stop,禁用', nullable=False, default=0)
    ...
    # 唯一索引
    __table_args__ = (
        UniqueConstraint('F_role_code', name='t_role_F_role_code'),
    )
